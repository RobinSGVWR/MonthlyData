...
public class ExcelWriterXSSF {
    ...
    ExcelWriterXSSF(String txt, XSSFWorkbook wb, List<List<String>> sheetL, ExcelReader excelR) {
        this.txt = txt;
        this.wb = wb;
        this.sheetL = sheetL;
        this.excelR = excelR;
    ...
    void generateExcel(){
        ...
        XSSFSheet sheet = wb.createSheet(this.txt);
...
//        stylePourcent.setDataFormat(wb.createDataFormat().getFormat("0.00%"));
        ...
        try {
            FileOutputStream fos = new FileOutputStream(System.getProperty("user.home") + "/Desktop/xslxCree.xlsx");
            wb.write(fos);
            fos.close();
            out.println("enregistré");
            this.excelR.addToConsole("le fichier a été enregistré");
        } catch (IOException e) {
            e.printStackTrace();
